...
for t in schema_table:
    ...
    SELECT column_name ,
        case when data_type ='character varying'
            then lower(data_type) || '(' || coalesce(character_maximum_length,0) || ')'
            when data_type = 'numeric'
            then 
                case
                    when coalesce(numeric_precision,0)=0
                        and coalesce(numeric_scale,0)=0
                            then lower(data_type)
                    when coalesce(numeric_precision,0)!=0
                        and coalesce(numeric_scale,0)=0
                            then lower(data_type) || '(' || numeric_precision || ')'
                    else lower(data_type)|| '(' || numeric_precision || ',' || numeric_scale || ')'
                    end
            when data_type = 'double precision'
                then ' double(' || numeric_precision || ')'
